...
MODES              = ['af_xdp-bp', 'af_xdp-poll']
FLAGS              = {
                      'af_xdp-bp': ['-B'],
                      'af_xdp-poll': ['-p']
                     }
APP_MODES          = ['HTS_sip','HTS_haraka','HTS_haraka_fnv']
APP_FLAGS          = {
                      'HTS_sip': ['-h', 'HSIPHASH'],
                      'HTS_haraka': ['-h', 'HARAKA'],
                      'HTS_haraka_fnv': ['-a']
...
out.write("run,mode,app_mode,throughput,target,llc-loads,llc-load-misses,llc-store,llc-store-misses,user,system,softirq,verified\n")
    ...
    for mode in MODES:
        ...
        for app_mode in APP_MODES:
                ...
                while max_t - min_t > TARGET_STEP:
                        ...
                        if app_mode != HTS_haraka_fnv:
                                cmd = ['ssh', TESTER, 'sudo', MOONGEN_PATH, PKTGEN_SCRIPT_PATH,
                                        '0', '0', '-c', '1', '-o', 'tmp.csv', '-r', str(curr_t),
                                        '-t', str(TRIAL_TIME)]
                        else:
                                cmd = ['ssh', TESTER, 'sudo', MOONGEN_PATH, PKTGEN_SCRIPT_PATH,
                                        '0', '0', '-c', '1', '-o', 'tmp.csv', '-r', str(curr_t),
                ...
                if app_mode != HTS_haraka_fnv: 
                        cmd = ['ssh', TESTER, 'sudo', MOONGEN_PATH, PKTGEN_SCRIPT_PATH, '0',
                                '0', '-c', '1', '-o', 'tmp.csv', '-r', str(best_t), '-t',
                                str(FINAL_TRIAL_TIME)]
                else:
                        cmd = ['ssh', TESTER, 'sudo', MOONGEN_PATH, PKTGEN_SCRIPT_PATH, '0',
                                '0', '-c', '1', '-o', 'tmp.csv', '-r', str(best_t), '-t',
                                str(FINAL_TRIAL_TIME), '-a']
                ...
                out.write(f'{run},{mode},{app_mode},{best_r},{best_t},{loads},{load_misses},{stores},{store_misses},{user},{system},{softirq},{verified}\n')
                out.flush()
